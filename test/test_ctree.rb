...
class TestCtree < Test::Unit::TestCase
  ...
  def test_session
    assert_nothing_raised { @session = CTDB::Session.new }
  ...
  def test_session_locking
    assert_nothing_raised do
      @session = CTDB::Session.new
      @session.logon(@c[:host], @c[:username], @c[:password])
    end
    assert(@session.lock(CTDB::LOCK_WRITE))
    assert(@session.locked?)
    assert(@session.unlock)
    assert_equal(false, @session.locked?)
    assert_nothing_raised{ @session.lock!(CTDB::LOCK_WRITE) }
    # assert_raise(CTDB::Error){ @session.lock!(CTDB::LOCK_READ) }
  ...
  def test_table
    ...
    assert_nothing_raised { @table = CTDB::Table.new(@session) }
  ...
  def test_record
    assert_nothing_raised do 
      @session = CTDB::Session.new
      @session.logon(@c[:host], @c[:username], @c[:password])
      @table = CTDB::Table.new(@session)
      @table.path = @table_path
      @table.open(@table_name)
    end
    assert_nothing_raised { @record = CTDB::Record.new(@table) }
